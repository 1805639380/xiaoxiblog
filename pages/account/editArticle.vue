  ...
  <nuxt-layout name="default" :user="userState">
    ...
    <nuxt-layout name="account" :active="1" :title="'编辑文章'">
      <template #account-wrap>
        <el-form :model="editForm" label-width="50px" :rules="rules" ref="ruleFormRef">
          <el-form-item label="标题:" prop="title">
            <el-input type="text" name="" id="text" v-model="editForm.title"></el-input>
          </el-form-item>
          <el-form-item label="类型:" prop="type">
            <client-only>
              <el-select v-model="editForm.type" class="m-2" placeholder="请选择文章类型" size="large">
                <el-option v-for="item in typeOptions" :key="item.value" :label="item.label" :value="item.value" />
              </el-select>
            </client-only>
          </el-form-item>
          <el-form-item label="图片:" prop="imageUrl">
            <el-upload class="avatar-uploader" name="pic" action="http://127.0.0.1:3000/upload/pic"
          ...
          <el-form-item label="摘要:" prop="snippet">
            <el-input type="text" name="" id="snippet" v-model="editForm.snippet"></el-input>
          </el-form-item>
          ...
          <el-form-item class="article-btn-wrap">
            <div class="article-my-btn">
              <el-button type="primary" size="default" @click="submitForm(ruleFormRef)">发布</el-button>
            </div>
          </el-form-item>
        </el-form>
      </template>
...
const handleAvatarSuccess: UploadProps['onSuccess'] = (
  ...
  if (response.status === 200) {
    editForm.imageUrl = response.data.filePath
  }
...
const rules = reactive({
  title: [{
    validator: nullCheck, trigger: 'blur'
  }],
  snippet: [{
    validator: nullCheck, trigger: 'blur'
  }],
  ...
  formEl.validate(async (valid) => {
    if (valid) {
      const { data } = await addArticle({
        ...
        article_content: editForm.content,
        article_type: editForm.type,
        article_snippet: editForm.snippet,
        article_pic: editForm.imageUrl
      })
